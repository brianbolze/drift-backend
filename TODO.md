...
- [ ] **Post-Nosler: batch curation pass to land parser-derived quality wins.** Store's `matched_existing` policy intentionally doesn't update already-stored rows, so the parser wins that are sitting in cache (Sierra `product_line` canonical values where DB has NULL, Nosler `product_line` from `Bullet Type` spec row where DB has NULL, Hornady/Sierra over-extracted cross-sell removals) need a deliberate curation step. Generate from `scripts/parser_vs_db_report.py` diffs. Batch across Hornady + Sierra + Nosler. (source: rollout review, 2026-04-22)
- [ ] **Sierra 35 cal 155gr Pro-Hunter G1 BC inconsistency.** Sierra's own page publishes two different G1 BC values: `0.176` in the primary BigCommerce attribute array, `0.182` in a secondary JSON-in-JSON block. DB currently has 0.182 (LLM picked the second block); parser reads 0.176 (primary array). This is a Sierra page defect — pick one via curation patch, don't let it create duplicate bc_sources on re-extraction. URL: `https://sierrabullets.com/35-cal-155-gr-fn-pro-hunter`. (source: smoke-test BC audit, 2026-04-22)
...
- [ ] **Nosler page typos flagged for curation.** Two Solid bullet pages have the caliber pasted into the `Bullet Weight` spec row instead of the real weight. Parser honestly reports what the page says; LLM had silently corrected both. URLs: `https://www.nosler.com/416-caliber-400gr-solid-25ct.html` (page says 416gr, should be 400), `https://www.nosler.com/458-caliber-500gr-solid-25ct.html` (page says 458gr, should be 500). DB already has correct values. Include in the batch curation pass as `is_locked=True` so re-extraction can't undo. (source: parser_vs_db_nosler.md, 2026-04-22)
- [ ] **Nosler cartridge → bullet_id=None on 37% (84/228).** The resolver finds a confident match for 63% of parser-extracted Nosler cartridges but returns `bullet_id=None` for the rest — bullet families the DB doesn't yet have (e.g. 30-378 Wby Mag 210gr AccuBond Long Range: the 30-cal 210gr AccuBond LR bullet exists in the DB but the resolver's cartridge→bullet matcher is called without a diameter filter, so it compares against a 338-cal 265gr variant instead). Two possible fixes per engineer's rollout review: (a) tighten the cartridge→bullet matcher to filter by diameter, (b) accept that some Nosler cartridges need manual bullet_id via curation. Zero sub-0.5 confidences, so nothing is silently shipping — the gate works. (source: parser_resolver_integration test output, 2026-04-22)
