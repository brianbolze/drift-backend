...
- [ ] **Post-Nosler: batch curation pass to land parser-derived quality wins.** Store's `matched_existing` policy intentionally doesn't update already-stored rows, so the parser wins that are sitting in cache (Sierra `product_line` canonical values where DB has NULL, Hornady/Sierra over-extracted cross-sell removals) need a deliberate curation step. Generate from `scripts/parser_vs_db_report.py` diffs. Batch across Hornady + Sierra + Nosler so it's one review instead of three. (source: rollout review, 2026-04-22)
