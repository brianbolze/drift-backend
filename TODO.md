...
- [ ] BLOCKER-v6: Product-line tier scoring regression in `match_bullet` — 25 of 37 same-diameter cartridge→bullet shifts are downgrades (SST→InterLock SP, ELD-X→ELD Match, Hornady ammo→Sierra bullets, Winchester Ballistic Silvertip→Power-Point with no BC, etc.). Classification + failure patterns + reproducer in `data/forensic/v6-resolver-regression-2026-04-22/product_line_tier_regression.md`. Must fix before v6 store-commit can be re-run. H3 fallback gate, caliber-compat across all tiers, and cross-diameter store guard are already landed and working correctly — this is a separate tier-scoring issue. (source: agent, 2026-04-22)
